...
- **Correlate mini-dashboard popup** (nico-ops specific) — PRD-007 Slice 0 (#373) + Slice 2 (#374). A centered Popup the operator opens with `[c]` on a focused row that mentions an entity ID; runs the **Correlate runner adapter** in-process against the shared ops bootstrap (no second kubeconfig load, no second port-forward) and incrementally renders a Diagnosis banner above a chronologically-sorted Event Timeline, with a source-availability dots row and an `[Enter] full   [esc] close` action row pinned at the bottom. State lives on `App::correlate` as `CorrelateState { entity, sources: Vec<(String, SourceStatus)>, events, source_errors, diagnosis, done }`; `SourceStatus` is `Loading` (`⟳`), `Landed` (`●`), `Failed` (`✗`), or `Unrequested` (`○`) — the dots row paints them in the order the runner emitted in `Loading { sources }`. Two reducer entry points: `Action::Correlate` (translator-emitted on `[c]`) extracts the first entity from the focused finding via `extract_entity_from_finding` and re-dispatches as `Action::OpenCorrelatePopup(EntityRef)`; the latter is the general path the per-surface triggers (Spotlight today; logs / Findings detail / event timeline in later slices) all funnel into. Spotlight surfaces a "no entity found in this row" toast when extraction returns nothing; Scorecard preserves the issue-#157 silent-inert contract for the workflow-only legacy path. Slice 2 (#374) replaced Slice 0's one-shot `Action::CorrelateResults` with streaming `Action::CorrelateUpdate { entity, update: CorrelateUpdate }` updates — the reducer accepts them in any order so a Diagnosis arriving before all events still keeps the view consistent.
- **Correlate runner adapter** (nico-ops specific) — PRD-007 Slice 2 (#374) / `crates/nico-ops/src/correlate_runner.rs`. Async streaming wrapper around `nico_correlate::collect_all`. `run_correlate(sources, id, id_type, diag_config) -> CorrelateStream` spawns one tokio task per Source and returns a `Stream<Item = CorrelateUpdate>` whose first update is always `Loading { sources }`, followed by per-Source landings in *completion* order (`SourceLanded { events, state }` for `Output`, `SourceFailed { reason }` for `Unavailable`), then `Diagnosis` computed over the accumulated events + state, then `Done`. Dropping the stream cancels in-flight tasks via a shared `CancellationToken` (cooperative `tokio::select!` on every per-Source task) — cancellation is the happy path: no error logged, no task leak. The host loop (`spawn_correlate` in `lib.rs`) consumes the stream, maps each correlate-crate `CorrelateUpdate` to its model-layer mirror, and dispatches as `Action::CorrelateUpdate { entity, update }` so the reducer can merge incrementally. Setup failures (`resolve_config` errors) surface through the same stream as a synthetic `SourceFailed { source: "config", reason }` so the popup paints them as a `source_error` row and shuts down the throbber via `Done`.
