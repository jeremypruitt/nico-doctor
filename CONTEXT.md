...
- **Correlate mini-dashboard popup** (nico-ops specific) — the centred `Popup`-primitive overlay that opens when the operator presses `[c]` on a focused row whose text contains an `EntityRef`. PRD-007 Slice 0 (#373) ships the tracer bullet: one trigger (Spotlight finding rows), one extracted entity type (DPU, via the slice-0 `extract_dpu_entity_from_finding` extractor — slice 1 deepens this into the full entity-extraction primitive across all four surfaces and entity types), and one render path. The body renders the **Diagnosis banner** at the top (a one-line `<pattern> — <error_signature>` summary, omitted when the pattern matcher returned `None`) followed by the **scrollable event timeline** (Source-attributed rows sorted chronologically, with failed Sources surfacing inline as synthetic `source_error` rows). Source-availability dots and the `[Enter] full` action row are intentionally absent in slice 0 — slice 2 adds the streaming runner adapter plus dots, slice 4 wires `[Enter]` for full-screen correlate. The reducer translates the `[c]` keystroke (`Action::Correlate`) into a typed `Action::OpenCorrelatePopup(EntityRef)` via `App::derive_correlate_action`; the host loop spawns the in-process correlate run and posts results back via `Action::CorrelateResults { entity, events, source_errors, diagnosis }`. Rows whose text yields no extractable entity raise a "no entity found in this row" toast. `Esc` (or `q`) dismisses; the existing `Overlay::Correlate` clear-on-close already wipes the popup state.
